...
+ (void)initialize
{
    checkedIcon = [UIImage imageNamed:@"Yes"];
...
- (void)setupButtons
{
    ...
    [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"返回", nil)
                                     style:UIBarButtonItemStylePlain
                                    target:self
                                    action:@selector(backPickingAssets:)];
...
- (void)finishPickingAssets:(id)sender
{
    ...
    if (photoArray.count) {
        [[NSNotificationCenter defaultCenter] postNotificationName:FinishSelectPicNotification object:photoArray];
...
- (void)backPickingAssets:(id)sender{
    ZYQAssetPickerController *picker = (ZYQAssetPickerController *)self.navigationController;
